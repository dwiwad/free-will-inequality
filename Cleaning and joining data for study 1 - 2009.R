...
mobility <- dplyr::select(mobility, Country, IGEincome, cohort) #remove other variables 
...
wvs_6df <- dplyr::filter(wvs_6df, Country %in% c("Cyprus", "Philippines")) #select countries missing from WVS 5
...
wvs_3df <- dplyr::filter(wvs_3df, Country  %in% c("Croatia", "Slovakia")) #select countries missing from WVS 5
...
#write.csv(mobility_merged, "FW Study 1.csv")
